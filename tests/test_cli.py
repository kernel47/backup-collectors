...
def test_cli_success_summary(monkeypatch, capsys):
    result = ExecutionResult("netbackup", "pamela", 2, 2, 2, "OK", 0.12)
    monkeypatch.setattr(cli, "execute", lambda context, settings, show_progress: result)
    code = cli.main(
        ["collect", "netbackup", "--scope", "pamela", "--asset", "master-01"]
    )
    assert code == 0
...
def test_expected_error_becomes_icinga_critical(monkeypatch, capsys):
    def fail(context, settings, show_progress):
...
def test_cli_can_force_pretty_progress(monkeypatch, capsys):
    ...
    def succeed(context, settings, show_progress):
